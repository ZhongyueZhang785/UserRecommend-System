...
import org.apache.spark.sql.functions.{array, col, collect_list, collect_set, concat, count, explode, flatten, lit, log, log10, lower, map_from_entries, rand, regexp_replace, reverse, round, row_number, size, struct, to_json, to_timestamp, udf, when}
...
object etlV2 {
  def main(args: Array[String]): Unit = {
    ...
    val replyRDD = tweetDF
      .select("user_id", "reply_to_id", "tweet_id")
      .filter(col("reply_to_id").isNotNull)
      .rdd.map(row => (row(0), row(1), row(2)))
      .map(x => {
    ...
    val replyDF = replyRDD
      .toDF("reply_user1_id", "reply_user2_id", "tweet_id")

    // group by user1_id and user2_id, generate reply_count and reply_tweet_id_list
    // [user1_id, user2_id, reply_count, reply_tweet_id_list]
    val replyDFAgg = replyDF.groupBy("reply_user1_id", "reply_user2_id").agg(
      count("tweet_id").alias("reply_count"),
      collect_set("tweet_id").alias("reply_tweet_id_list")
    )
    ...
    val retweetRDD = tweetDF
      .select("user_id", "retweet_to_id", "tweet_id")
      .filter(col("retweet_to_id").isNotNull)
      .rdd.map(row => (row(0), row(1), row(2)))
      .map(x => {
        //        val user_id = BigInt(x._1.toString)
        //        val retweet_to_id = BigInt(x._2.toString)
        val user_id = x._1.toString
        val retweet_to_id = x._2.toString
        if (user_id <= retweet_to_id) {
          (x._1, x._2, x._3)
        } else {
          (x._2, x._1, x._3)
        }
    ...
    val retweetDF = retweetRDD
      .toDF("retweet_user1_id", "retweet_user2_id", "tweet_id")

    // group by user1_id and user2_id, generate reply_count and reply_tweet_id_list
    // [user1_id, user2_id, reply_count, reply_tweet_id_list]
    val retweetDFAgg = retweetDF.groupBy("retweet_user1_id", "retweet_user2_id").agg(
      count("tweet_id").alias("retweet_count"),
      collect_set("tweet_id").alias("retweet_tweet_id_list")
    )

    // Contact Table
    // [rowkey(reverse(user1_id|user2_id)), user1_id, user2_id, reply_count, reply_tweet_id_list, retweet_count,
    // retweet_tweet_id_list, interaction_score]
    ...
    val contactDF = contactDFJoinReply.join(
      ...
      .withColumn("reply_tweet_id_list", when(
        col("reply_tweet_id_list").isNull, array()
      ).otherwise(col("reply_tweet_id_list")))
      .withColumn("retweet_tweet_id_list", when(
        col("retweet_tweet_id_list").isNull, array()
      ).otherwise(col("retweet_tweet_id_list")))
    ...
    contactDF
      //      .select("HBASE_ROW_KEY", "user1_id", "user2_id", "interaction_score", "reply_count", "reply_tweet_id_list", "retweet_count", "retweet_tweet_id_list")
      .select("HBASE_ROW_KEY", "interaction_score", "reply_tweet_id_list", "retweet_tweet_id_list")
      .withColumn("reply_tweet_id_list", arrayToStringUdf(col("reply_tweet_id_list")))
      .withColumn("retweet_tweet_id_list", arrayToStringUdf(col("retweet_tweet_id_list")))
      .coalesce(1)
      .write
      .option("compression", "gzip")
      .option("delimiter", "\t")
